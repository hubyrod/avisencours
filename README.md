...
5. Reads a fourth source, **Maximilien** (marches.maximilien.fr, the shared buyer profile of Île-de-France public bodies, an atexo MPE / PRADO platform): the full list of open consultations (~450, 20 per page, each next page a PRADO postback replaying the page state with the session cookie) is filtered locally on SERVICES + the client's Île-de-France keyword list (`MAXIMILIEN_SEARCHES` in `src/defaults.ts`, families mobilité and inondations); detail pages (public) are fetched only for retained consultations (CPV, type d'annonce). `MAXIMILIEN=0` disables it.
...
src/
  ...
  maximilien.ts       Fourth source: Maximilien (Île-de-France, atexo MPE): PRADO postback pagination, row + fiche parsers
