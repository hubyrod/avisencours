...
3. Reads a second source, the **achatpublic.com** buyer-profile platform (where many sub-threshold MAPA studies are published without ever reaching BOAMP): every open SERVICES and FOURNITURES consultation is listed (~120 form POSTs, 10 cards each, ~2–3 minutes) and matched locally against per-family keyword lists in `src/defaults.ts` (`ACHATPUBLIC_SEARCHES`, mirroring the consultancy's manual watch procedure): **mobilité** (services, classified like BOAMP avis), **kiomda** (fournitures — counters/sensors for bike and pedestrian counting) and **inondations** (services — AMC / PAPI / GEMAPI flood-prevention studies). The two non-mobility families are classified by their own rule in `src/familles.ts`, never by the LLM. Only retained consultations have their public detail page fetched (description, CPV, opening date). The source is non-blocking: if the site is down, the run keeps the BOAMP results and records a warning. `ACHATPUBLIC=0` disables it.
4. Reads a third source, the **AFD notices on dgMarket** (afd.dgmarket.com, category « Transports », open notices only): list, category filter and full notice text are public (only document downloads need a subscription); the first request goes through an anonymous auto-login redirect chain that sets a session cookie, handled by a small per-domain cookie jar in `src/afd.ts`. Notices are abroad and often in English; they go through the normal mobility classifiers. About 10 requests per run. `AFD=0` disables it.
5. Reads the **atexo MPE platforms** listed in `MPE_SITES` (`src/mpe.ts`): **Maximilien** (marches.maximilien.fr, Île-de-France, ~450 open consultations) and **AMPA** (demat-ampa.fr, Marchés publics d'Aquitaine, ~300). Same software, same HTML: the full list of open consultations (20 per page, each next page a PRADO postback replaying the page state with the session cookie) is filtered locally on SERVICES + the client's keyword list (`MPE_SEARCHES` in `src/defaults.ts`, families mobilité and inondations); detail pages (public) are fetched only for retained consultations (CPV, type d'annonce). `MAXIMILIEN=0` / `AMPA=0` disable a site; adding another MPE site (marches-publics.gouv.fr is one) is one entry in `MPE_SITES`.
6. Reads **Marchés Online** (marchesonline.com, Le Moniteur's aggregator, "en accès libre"): no account for what we use — the public keyword pages (`/appels-offres/top-recherches/<mot>`, 20 per page) are walked for every keyword of the client's five search profiles (`MARCHESONLINE_SEARCHES` in `src/defaults.ts`: mobilité, kiomda counters, inondations; ~250 pages per run) and filtered locally on the card's activity label (services / études vs fournitures) and on the keywords at start of word (the site's search is lexical). Public detail pages are read only for retained notices the title alone does not already exclude (descriptif, CPV). The site rate-limits (Cloudflare), so requests are paced and wide keywords are capped to their newest pages (~3–4 minutes per run once known notices are reused; ~15 minutes the first time). `MARCHESONLINE=0` disables it.
7. Classifies each avis into `relevant` / `travaux` / `excluded`. Writes two timestamped markdown reports per run (`avis-en-cours-YYYY-MM-DD-HH-MM-SS.md` and the travaux equivalent). Excluded avis are logged to stderr with their reason.

Deployed, the same pipeline runs daily into PostgreSQL behind a French dashboard (passwordless email-code login). Each avis has a detail page with a **live comment thread** and a **collaborative status** (« à évaluer », « répondu », « gagné »… — list editable by admins on `/admin`, every change kept as history in the thread). The dashboard's primary click opens the internal detail page (the official notice stays one click away), lets you change an avis' status from the list, filters by text, status, source (with counts) and family, sorts by deadline or recency, marks what is new since your previous visit, applies filters as you change them, and can hide rejected avis; the same appel d'offres published on several platforms (or republished / rectified on BOAMP) is grouped into one row listing every publication, with one status and one comment thread; `/configuration` lists the sources (what each reads, active/disabled/failed, current avis per source) and, while an update runs, shows its milestones (one step per source, then classification, then the email) with counters and a progress bar, updated in place through the Skip engine (the job writes `runs.progress`, the adapter watches `runs`, the page refetches a server-rendered fragment on each event); the dashboard banner carries a one-line summary the same way. `/configuration` (admins, or users delegated the right from `/admin`) edits the pipeline live: search keywords, « toujours garder / toujours exclure » rules, classifier mode, LLM model chain (with live OpenRouter prices), digest window, départements, plus a « Relancer maintenant » button. Threads update in real time through the [Skip](https://skiplabs.io) reactive engine running in-process (WASM) on Postgres LISTEN/NOTIFY; set `LIVE_COMMENTS=0` to disable it (threads then show as unavailable). `/sante` reports `live` (engine up) and `livePg` (its Postgres connection + number of watched tables).
...
All sources are read in parallel, so a run lasts as long as its slowest source (Marchés Online, a few minutes). A full run hits the portal API ~30 times (the first page alone, then four pages at a time with retry; ~3 000 records over a 14-keyword query), re-reads a notice's detail page only when its card changed (URL/version, title or deadline — the rest is reused from the previous run), and only asks the LLM about avis it has not already classified with the same text, prompt and model chain (the verdict is stored with a content key in `announcements.llm_key`; `LLM_MEMO=0` forces a full reclassification). The first run sends ~2 000 avis to the LLM, about 2.4 M prompt tokens — roughly 5 cents of OpenRouter credit with the default chain (mistral-nemo, 39/40 on `bun run eval` and the cheapest JSON-capable model in the catalog; the previous llama-3.3-70b head cost ~27 cents per run) when the hybrid classifier is on (the exact cost, token counts and which models answered are recorded on each run and shown on `/configuration` and in the digest footer). Each call carries the whole model chain: OpenRouter switches to the next model on outage, rate limit, moderation or context overflow, and the client retries timeouts / 5xx and re-asks the rest of the chain once when a model returns unusable JSON. A dead key, an empty credit or five consecutive chain failures open a circuit breaker: the rest of the run is classified by regex, the run is flagged with a warning, `ALERT_RECIPIENT` gets an email, and the digest is held until the next healthy run.
...
src/
  ...
  scraper.ts          Paginated fetch loop + ODS record → Announcement mapping (BOAMP)
  achatpublic.ts      Second source: achatpublic.com form scraper (session cookie, 0-based pages, card + fiche parsers)
  afd.ts              Third source: AFD notices on dgMarket, « Transports » category (cookie jar, redirect chain, list + notice parsers)
  progress.ts         Run progress: ProgressTracker (steps, throttled writes), summary/percent helpers
  sources.ts          Registry of sources (label, URL, description, env kill switch) for the pipeline, dashboard filter and configuration page
  mpe.ts              atexo MPE platforms (Maximilien, AMPA…): PRADO postback pagination, row + fiche parsers, MPE_SITES registry
  marchesonline.ts    Marchés Online (Le Moniteur): public keyword pages walked per keyword, card + fiche parsers
  mois.ts             Month name (FR/EN, short/long) → « 01 »…« 12 », shared by the sources with dates in words
  familles.ts         Watch families (mobilité / kiomda / inondations): keyword matcher + per-family classification rule
  departements.ts     Département name → INSEE code (achatpublic only exposes the label)
